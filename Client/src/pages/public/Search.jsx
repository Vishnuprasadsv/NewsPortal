...
const Search = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/news?status=Published&keyword=${encodeURIComponent(query)}`);
        setNews(data.news);
      } catch (error) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 pt-12 pb-16">
      <div className="border-b-[2px] border-black pb-4 mb-8">
        <h1 className="font-serif text-3xl">Search Results for: "{query}"</h1>
      </div>
      
      {loading ? (
        <div className="loader mt-8"></div>
      ) : news.length === 0 ? (
        <p className="text-muted font-semibold">No published articles found matching your query.</p>
      ) : (
